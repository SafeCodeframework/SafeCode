...
class AdversarialTrainingAttackerRandom(object):
    ...
    def gen_all(self, n_iter=20, res_save=None, adv_sample_size=5000):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-iter', default=iteration_times, type=int)
    ...
    max_len = 300
    domain = 'origin'
    root = '/home/yjy/code/2023/paper01_data/code_function/'
    model_path = os.path.join(root, 'model', 'LSTM', 'origin', '15.pt')
    adv_save_path = os.path.join(root, 'data', 'adv', 'LSTM', str(opt.iter))
    os.makedirs(adv_save_path, exist_ok=True)
    poj = OJ104(path=root + "data/" + domain + "/oj.pkl.gz",
                max_len=max_len,
    ...
    with gzip.open(root + 'data/' + domain + '/data_uid.pkl.gz', "rb") as f:
        symtab = pickle.load(f)
    with gzip.open(root + 'data/' + domain + '/data_inspos.pkl.gz', "rb") as f:
    ...
    atk.attack_all(40, 50,
                   res_save=adv_save_path + "/token.pkl.gz",
                   adv_sample_size=2000)

    atk = AdversarialTrainingInsAttacker(poj, instab, classifier)
    atk.attack_all(40, 20, res_save=adv_save_path + "/dead_code.pkl.gz",
                   adv_sample_size=2000)
