...
def token_to_code(tokens):
    code = ''
    indent = 0
    for token in tokens:
        if token == '{':
            indent += 1
            code += ' ' * (indent - 1) * 4 + token + '\n'
        elif token == '}':
            indent -= 1
            code += ' ' * indent * 4 + token + '\n'
        else:
...
class TokenModifier(object):
    ...
    def __init__(self, classifier, loss, uids, txt2idx, idx2txt, device):

        self.device = device
        self.cl = classifier.to(self.device)
    ...
    def __gen_uid_mask_on_vocab(self, uids):
        ...
        _uids = _uids.reshape([self.cl.vocab_size, 1]).to(self.device)
        return _uids
    ...
    def __gen_uid_mask_on_seq(self, uids):
        ...
        _uids.index_put_([torch.LongTensor(uids)], torch.Tensor([1 for _ in uids]))
        _uids = _uids.reshape([self.cl.max_len, 1]).to(self.device)
        return _uids
    ...
    def rename_uid(self, x, y, x_uid, ori_uid, n_candidate=5):
        ...
        _x = torch.tensor(np.array(x), dtype=torch.long).to(self.device).permute([1, 0])
        _y = torch.tensor(y, dtype=torch.long).to(self.device)
        grad = self.cl.grad(_x, _y, self.loss.to(self.device))[0]
    ...
    def rename_uid_mu(self, x, y, x_uid, ori_uid, n_candidate=5, noise_scale=0):
        noise_scale = ((noise_scale - 1) * 4) % 11
        _x_uid = []
        for i in x_uid:
            if i < self.cl.max_len:
                _x_uid.append(i)
        ...
        _y = torch.tensor(y, dtype=torch.long).to(self.device)
        grad = self.cl.grad(_x, _y, self.loss.to(self.device))[0]
        noise = torch.randn(grad.shape, device=self.device) * noise_scale * 0.05  # noise_scale is the standard deviation of the noise
        grad += noise
        mean_grad = torch.sum(grad * x_uid, dim=0) / torch.sum(x_uid)
        mean_grad = mean_grad.reshape([1, self.cl.embedding_size])
        ori_embed = self.cl.embedding.weight[ori_uid] \
    ...
    def rename_uid_dis(self, x, y, x_uid, ori_uid, n_candidate=5):
        _x_uid = []
        for i in x_uid:
            if i < self.cl.max_len:
                _x_uid.append(i)
        ...
        mean_grad_exp = mean_grad.expand_as(delta_embed)
        dist = torch.sum((mean_grad_exp - delta_embed) ** 2, dim=1)

        # inner_prod = torch.sum(mean_grad * delta_embed, dim=1) / delta_embed_len
        # mutual_info = torch.log(torch.exp(inner_prod) / torch.sum(torch.exp(inner_prod)))

        _, new_uid_cand = torch.topk(dist, n_candidate)
        new_uid_cand = new_uid_cand.cpu().numpy()
        new_x = []
        for new_uid in new_uid_cand:
            new_x.append(copy.deepcopy(x[0]))
    ...
    def rename_uid_random_cand(self, x, x_uid, ori_uid, n_candidate=5):
...
def mutual_information(x, y, bins=30):
    mi = torch.zeros(y.shape[0])
    x = x.detach().cpu().numpy().squeeze()  # Convert x to numpy array for processing with numpy
    for i in range(y.shape[0]):
        y_i = y[i].detach().cpu().numpy()  # Convert y[i] to numpy array for processing with numpy
        hist_2d, _, _ = np.histogram2d(x, y_i, bins=bins)
        ...
        mi[i] = np.sum(pxy[nzs] * np.log(pxy[nzs] / px_py[nzs]))
    return mi  # mi is already a torch tensor
...
#     max_len = 300
#
#     poj = OJ104(path="../dataset/oj.pkl.gz", max_len=max_len, vocab_size=vocab_size)
#     training_set, valid_set, test_set = poj.train, poj.dev, poj.test
#     with gzip.open('../dataset/data_uid.pkl.gz', "rb") as f:
#         symtab = pickle.load(f)
#     with gzip.open('../dataset/data_inspos.pkl.gz', "rb") as f:
...
#                                 hidden_size, num_classes, max_len, attn=opt.attn).to(device)
...
#
#     x = torch.tensor(b['x'], dtype=torch.long).to(self.device).permute([1, 0])
#     y = torch.tensor(b['y'], dtype=torch.long).to(self.device)
#     print (b['y'][0])
...
#         new_prob = classifier.prob(torch.tensor(new_x, dtype=torch.long).to(self.device).permute([1, 0]))
...
#
#     x = torch.tensor(b['x'], dtype=torch.long).to(device).permute([1, 0])
#     y = torch.tensor(b['y'], dtype=torch.long).to(device)
#     print(b['y'][0])
#     prob = classifier.prob(x)[0]
#     print(int(torch.argmax(prob)), float(prob[b['y'][0]]))
#
...
#         new_prob = classifier.prob(torch.tensor(feed_new_x, dtype=torch.long).to(device).permute([1, 0]))
