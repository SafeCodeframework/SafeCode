...
if __name__ == "__main__":
    ...
    dir = './tmp'
    tgt = '../dataset/oj.pkl.gz'
    symtab = '../dataset/data_uid.pkl.gz'
    done_file = 'dataset.done'
    inspos_file = '../dataset/data_inspos.pkl.gz'
        ...
        if d is not None:
            ...
            if not os.path.isfile(os.path.join(dir, done_file)):
                ...
                with open(os.path.join(dir, done_file), "wb") as f:
                    pass
                stmt_poses_tr = [StmtInsPos(tr) for tr in train['raw']]
                stmt_poses_te = [StmtInsPos(te) for te in test['raw']]
                decl_poses_tr = [DeclInsPos(tr) for tr in train['raw']]
                decl_poses_te = [DeclInsPos(te) for te in test['raw']]
