...
def IfElseReplace(tokens, pos):
    ...
    res = beforeIf + ["if", "(", "!", "("] + codition + [")", ")", "{"] + blockElse + ["}", "else", "{"] + blockIf + [
        "}"] + afterElse
    return res
    ...
    for i, t in enumerate(tokens):
        if t == "if":
            ...
            if tokens[conditionEndIdx + 1] == "{":
                ...
                ifBlockEndIdx = _go4next(tokens, ";", conditionEndIdx + 1)
            if ifBlockEndIdx + 1 < n and tokens[
                ifBlockEndIdx + 1] == "else":  # in case of "if {} else {}", only process "if {} xxx"
                continue
...
def For2WhileRepalce(tokens, pos):
    ...
    if beforeFor != [] and beforeFor[-1] in [";", "{", "}"]:
        ...
        res = beforeFor + ["{"] + condition1 + ["while", "("] + condition2 + [")", "{"] + body + condition3 + ["}",
                                                                                                               "}"] + afterFor
    return res
...
if __name__ == "__main__":
    ...
    with gzip.open('../dataset/oj.pkl.gz', "rb") as f:
        d = pickle.load(f)
